...
const RightSidebar: React.FC<SidebarProps> = ({ isOpen, content, toggleSidebar }) => {
  return (
    <div
      ...
      } w-96 flex`} style={{top: '5rem'}}// 사이드바와 아이콘을 함께 움직이도록 설정 (사이드바 너비 조정: 80px)
    >
      {/* 사이드바 아이콘 부분 */}
          ...
          <div>
            ...
            <div className='mb-16'>
              <div className='flex mr-3 items-center gap-2 mb-4'>
                <img src='/assets/SpaceCloud-Logo.png' className='w-10 h-10'/>
                <h3 className="text-lg font-bold items-center">스페이스 클라우드</h3>
              </div>
              <input type="text" className="w-full p-2 border rounded mb-2" placeholder="아이디" />
              <input type="password" className="w-full p-2 border rounded mb-2" placeholder="비밀번호" />
              <button className="btn bg-gray-300 text-gray-700 px-4 py-2 rounded-lg w-full">계정 저장</button>
              <span className='flex gap-2 text-gray-500 text-xs mt-4'>플랫폼 방문하여 아이디/비밀번호 찾기 <BsChevronRight /></span>
            </div>
            <div className='mb-16'>
              <div className='flex mr-3 items-center gap-2 mb-4'>
                <img src='/assets/OurPlace-Logo.png' className='w-10 h-10'/>
